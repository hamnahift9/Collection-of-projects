...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        """
        #Marks cell as a move and as safe
        self.moves_made.update([cell])

        if cell not in self.safes:
            self.safes.update([cell])
        
        #Adds new sentence to KB
        nhbr_cells = set()

        for row in range(cell[0]-1,cell[0]+2):
            for clmn in range(cell[1]-1,cell[1] + 2):
                if 0 <= row < self.height and 0 <= clmn < self.width:
                    if (row,clmn) != cell:
                        nhbr_cells.update([(row,clmn)])
            clmn += 1 
        row += 1

        nhbr_cells -= self.moves_made
        new_sentence = Sentence(nhbr_cells,count)
        ...
        for sentence in self.knowledge:
            if len(sentence.cells) == 0:
                ...
                created_sentence= Sentence(new_set,new_count)
                knowledge.append(created_sentence)
    ...
    def make_random_move(self):
        ...
        """
        if len(self.moves_made) == 64:
            return None

        all_cells = set()
        for i in range(self.height):
            for j in range(self.width):
                all_cells.update([(i,j)])

        safe_moves = all_cells - self.mines
        possible_moves = safe_moves - self.moves_made
